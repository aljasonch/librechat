...
export const getRehypePlugins = (animateWords = false): PluggableList => {
  if (!animateWords) {
    return getBaseRehypePlugins();
  }
  if (streamingRehypePluginsCache === null) {
    streamingRehypePluginsCache = [...getBaseRehypePlugins(), rehypeStreamingWords];
  }
  return streamingRehypePluginsCache;
};
